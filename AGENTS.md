...
## Agreement Score Rules
- 협정보드의 경영점수, 재무비율, 신용평가, 영업기간 계산은 가능한 한 공통 평가 로직을 사용한다.
- 새 발주처나 금액대를 추가할 때 발주처별/금액대별 차이는 별도 계산 함수를 새로 만들기보다 설정값, 배점표, 기준표, 만점, 반올림 규칙으로 반영한다.
- 부채비율과 유동비율처럼 여러 발주처가 공유하는 판정 방식은 공통 로직을 우선 사용하고, 특정 구간 전용 산식을 복사해 구현하지 않는다.
- 신용평가가 없거나 유효기간이 만료된 경우 재무비율 평가로 전환하는 흐름도 공통 로직을 따른다.
- 신용평가 만료 업체에 남아 있는 기존 신용점수, 경영점수, 캐시 값이 재무비율 평가 결과를 덮어쓰지 않도록 주의한다.
- 신인도 가점은 사용자가 별도 상한값을 명시하거나 기준 문서/설정에 상한이 명확히 있는 경우에만 상한을 적용한다.
- 신인도 상한값이 `null`, `undefined`, 빈 값이면 0점 상한이 아니라 "상한 없음"으로 처리한다. `Number(null) === 0` 같은 변환으로 신인도 반영점수를 0점으로 잘라내지 않는다.
- 신인도 개별 반영점수와 신인도 합계는 같은 산식과 같은 상한 처리 기준을 사용해야 한다. 예: 국가철도공단 50억 미만은 `신인도 × 지분 × 0.5/3`이며, 신인도 3점과 지분 80%는 0.40점으로 반영된다.
- 특정 발주처/금액대에서 공통 로직으로 처리하기 어려운 예외가 있으면, 임의로 전용 분기를 만들지 말고 예외 사유와 다른 구간에 미치는 영향 범위를 먼저 확인한다.
