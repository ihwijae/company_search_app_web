...
const getStatusFromColor = (cell) => {
  if (!cell || !cell.style || !cell.style.fill) return '1년 이상 경과';
...
const getCreditGradeRank = (gradeToken) => {
  ...
  return CREDIT_GRADE_RANK.has(normalized) ? CREDIT_GRADE_RANK.get(normalized) : Number.POSITIVE_INFINITY;
};
...
const isWomenOwned = (company) => {
  const raw = company && company['여성기업'];
...
const hasQualityEvaluation = (company) => {
  const raw = company && company['품질평가'];
  if (raw === null || raw === undefined) return false;
  const text = String(raw).trim();
  if (!text) return false;
  const normalized = text.replace(/\s+/g, '').toLowerCase();
...
const buildDedupKey = (item) => {
  ...
  const regionToken = item['대표지역'] || item['지역'] ? String(item['대표지역'] || item['지역']).trim() : '';
  const appendRegion = (baseKey) => (regionToken ? `${baseKey}|region:${regionToken}` : baseKey);
  const biz = normalizeBizNumber(item['사업자번호']);
  if (biz) {
    return appendRegion(typeToken ? `biz:${biz}|type:${typeToken}` : `biz:${biz}`);
  }
  const nameSource = item['검색된 회사'] || item['회사명'] || item['대표자'];
  if (nameSource) {
    const normalized = String(nameSource).trim();
    if (normalized) {
      return appendRegion(typeToken ? `name:${normalized}|type:${typeToken}` : `name:${normalized}`);
    }
  }
  return '';
};
...
function postProcessResults(inputResults, options = {}) {
  ...
  let working = [...base];
  if (onlyLatest) working = working.filter((item) => (item && item['요약상태']) === '최신');
  ...
  if (sortField) {
    ...
    working = [...working].sort((a, b) => {
      const rawA = a && a[sortField];
      const rawB = b && b[sortField];
      ...
      if (av === bv) {
        const nameCompare = collator.compare(String((a && a['검색된 회사']) || ''), String((b && b['검색된 회사']) || ''));
        if (nameCompare !== 0) return nameCompare;
        return collator.compare(String((a && a['사업자번호']) || ''), String((b && b['사업자번호']) || ''));
      }
...
function searchCompaniesInDataset(companies, criteria = {}, options = {}) {
  ...
  if (minCreditGrade && Number.isFinite(minCreditRank)) {
    results = results.filter((company) => {
      const effectiveRank = Number.isFinite(company && company._creditGradeRank)
        ? company._creditGradeRank
        : getCreditGradeRank(extractCreditGradeToken(company && company['신용평가']));
...
async function extractCompaniesWithExcelJs(buffer, fileType, fileName) {
    ...
    for (let rIdx = 1; rIdx <= maxRow; rIdx += 1) {
      ...
      for (let cIdx = 2; cIdx <= maxCol; cIdx += 1) {
        ...
        Object.keys(RELATIVE_OFFSETS).forEach((item) => {
          ...
          companyStatuses[item] = getStatusFromColor(valueCell);
        });
  ...
  return { type: fileType, fileName: fileName || '', sheetNames, companies };
}

function extractCompaniesWithXlsx(buffer, fileType, fileName) {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    for (let row = range.s.r; row <= range.e.r; row += 1) {
      ...
      for (let col = 1; col <= range.e.c; col += 1) {
        ...
        const rawCompanyName = normalizeCellText((companyCell && (companyCell.v ?? companyCell.w)) || '');
        ...
        Object.keys(RELATIVE_OFFSETS).forEach((item) => {
          ...
          let processedValue = cell ? (cell.v ?? cell.w ?? '') : '';
...
async function parseDatasetBuffer(buffer, fileType, fileName) {
  try {
    return await extractCompaniesWithExcelJs(buffer, fileType, fileName);
  } catch (error) {
    console.warn('[dataset-parser] exceljs parse failed, fallback to xlsx:', error && error.message ? error.message : error);
    return extractCompaniesWithXlsx(buffer, fileType, fileName);
  }
}
