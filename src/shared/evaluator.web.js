import formulasDefaults from './formulas.defaults.json';
...
export function applyRounding(num, rounding) {
...
function evalManagement(inputs, rules, industryAvg) {
  ...
  const result = selection === 'max'
    ? (composite.score >= credit.score
...
function evalPerformance(inputs, rules) {
  ...
  if (perf.mode === 'ratio-bands') {
    if (thresholdMax > 0 || configMaxScore != null) {
      const best = Math.max(thresholdMax > 0 ? thresholdMax : 0, configMaxScore != null ? configMaxScore : 0);
...
export function pickTierByAmount(tiers = [], amount) {
  const a = toNumber(amount);
  if (!Number.isFinite(a)) return tiers && tiers[0];
  return tiers.find((t) => a >= toNumber(t.minAmount) && a < toNumber(t.maxAmount)) || tiers[0];
}

export function evaluateScores({ agencyId, amount, inputs = {}, industryAvg, formulasDoc } = {}) {
  const formulas = formulasDoc && typeof formulasDoc === 'object' ? formulasDoc : formulasDefaults;
  ...
  const rules = { ...(tier.rules || {}), agencyId: agency.id };
