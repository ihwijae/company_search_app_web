...
export const webSearchStore = {
  ...
  async clearDatasets(types = DATASET_TYPES) {
    ...
    await Promise.all(normalizedTypes.map(async (type) => {
      try {
        await removePersistedDataset(type);
      } catch (error) {
        console.warn('[webSearchStore] dataset clear failed:', error);
      }
    }));
    notifyListeners({ type: 'all', source: 'clear' });
    return { success: true };
  },
