...
/**
 * DebtModal: 부채비율 임계값 편집 UI
 * props:
 * - open, onClose
 * - rows: [{ op: 'lt'|'gte', value: number, score: number }]
 * - onSave: (rows) => void | Promise<void>
 */
export default function DebtModal({ open, onClose, onSave, onRestore, onReload, rows = [] }) {
  ...
  const addRow = () => {
    // 값은 % 단위로 편집(예: 50은 50%)
    setItems(prev => [...prev, { op: 'lt', value: 100, score: 0 }]);
  };
    ...
    >
      <div>
        ...
        <p ref={focusRef} tabIndex={-1} style={{ marginTop: 0 }}>
          부채비율 임계값 테이블을 편집합니다. 행 순서가 위에서 아래로 평가됩니다.
        </p>
        {error && (
                ...
                <tr key={i}>
                  ...
                  <td>
                    <input className="filter-input" type="number" step="1" value={r.value}
                      onChange={(e)=>setField(i, 'value', e.target.value)} placeholder="예: 50 → 50%" />
                  </td>
                  <td>
