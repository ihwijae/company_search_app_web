...
/**
 * QualityModal: 품질평가 임계값 편집 UI
 * props:
 * - open, onClose
 * - rows: [{ op: 'gte'|'lt', value: number, score: number }]
 * - onSave: (rows) => void | Promise<void>
 * - onRestore, onReload: actions
 */
export default function QualityModal({ open, onClose, onSave, onRestore, onReload, rows = [] }) {
  ...
  const addRow = () => {
    setItems(prev => [...prev, { op: 'gte', value: 80, score: 1 }]);
  };
  ...
  const move = (idx, dir) => {
    setItems(prev => {
      ...
      const t = next[idx]; next[idx] = next[j]; next[j] = t; return next;
    });
  };
  ...
  const validate = () => {
    for (let i = 0; i < items.length; i++) {
      ...
      const v = Number(r.value); const s = Number(r.score);
  ...
  const handleSave = () => {
    ...
    const normalized = items.map(r => ({ op: r.op, value: Number(r.value), score: Number(r.score) }));
    ...
    >
      <div>
        ...
        <p ref={focusRef} tabIndex={-1} style={{ marginTop: 0 }}>
          품질평가 점수 기준을 편집합니다. 값은 절대 점수(예: 88, 85)입니다.
        </p>
            ...
            <tbody>
              {(items || []).map((r, i) => (
                <tr key={i}>
                  ...
                  <td>
                    <input className="filter-input" type="number" step="0.1" value={r.value}
